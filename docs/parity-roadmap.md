...
| P3-06 | Weitere Channels | Buzz (Nostr), ClickClack ✅, IRC ✅, Email ✅, Mattermost ✅, Google Chat ✅, Synology Chat ✅, Feishu ✅, X ✅ u. a. | 🟢 | 🟡 |
...
| **0.4.0** | Channels | P3-06 Weitere Channels (IRC ✅, Email ✅, Mattermost ✅, Google Chat ✅, Feishu ✅, Synology Chat ✅, X ✅, ClickClack ✅), P3-07 Media-Message-Support, P3-08 Ingress-Monitor ✅, ~~P2-05~~ ✅ Control-UI | Nutzbares Multi-Plattform-Produkt |
...
> **E-Mail-Adapter (P3-06) getroffen:** `EmailChannelAdapter` implementiert den `ChannelAdapter`-Port für **E-Mail** über RFC 5321 (SMTP, Senden) und RFC 3501 (IMAP, Empfang). Konfiguration im `Channel.config`: `server` (Mail-Host, Pflicht), `username`/`password` (Pflicht), `from` (optional, Standard `username`), `smtpPort` (Standard 25), `imapPort` (Standard 143), `imapFolder` (Standard `INBOX`), `pollIntervalSeconds` (Standard 30), `subject` (Standard `JClaw`), `useTls` (optional, Implicit-TLS). Senden: SMTP-Dialog (`EHLO`/`MAIL FROM`/`RCPT TO`/`DATA` mit From/To/Subject/Date + Dot-Stuffing/`QUIT`); Empfänger aus `threadId`/`senderId`. Empfang: IMAP-Polling im Daemon-Thread (`LOGIN`/`SELECT`/`UID SEARCH UNSEEN`/`UID FETCH … (RFC822)`/`UID STORE … +FLAGS (\Seen)`); eingehende Nachrichten werden mit Jakarta Mail (`MimeMessage`) geparst (text/plain bevorzugt, HTML-Fallback, Multipart-Rekursion) und in `ChannelMessage.inbound` umgesetzt (content=Body, senderId/senderName=From, threadId=From-Adresse, externalId=UID). Neue Dependency: `spring-boot-starter-mail` (Jakarta Mail, Spring-Boot-verwaltet) — die SMTP-/IMAP-Verbindungen selbst nutzen Standard-Java-Sockets mit roher Literal-Verarbeitung. Adapter ist als `@Component` mit `@ConditionalOnProperty(jclaw.channels.enabled=true)` registriert. 12 Tests (Verfügbarkeit, SMTP-Dialog inkl. DATA/Dot-Stuffing, Empfänger-Fallbacks, SMTP-Fehler, IMAP-Polling-End-to-End gegen Fake-IMAP-Server, MIME-Parsing inkl. Multipart).

> **Mattermost-Adapter (P3-06) getroffen:** `MattermostChannelAdapter` implementiert den `ChannelAdapter`-Port für **Mattermost** über Webhooks. Konfiguration im `Channel.config`: `incomingWebhookUrl` (Pflicht zum Senden, URL enthält das geheime Hook-Id), `outgoingWebhookToken` (optional, für die Empfangs-Verifikation), `channel` (optionaler Channel-Override im Payload, z. B. `town-square`), `username` (optionaler Anzeigename des Bots). Senden: `POST` des JSON-Payloads `{"channel":…,"username":…,"text":…}` an den Incoming Webhook — `channel` aus `threadId`/`senderId` (wird weggelassen, wenn kein Ziel gesetzt ist → Webhook-Default), bei HTTP ≥ 400 wird eine `ChannelException` geworfen. Empfang ist **push-basiert** (Mattermost-Outgoing-Webhook) — `startReceiving`/`stopReceiving` bleiben Default-No-ops; der Adapter stellt `verifyWebhook()` (Token-Check gegen `outgoingWebhookToken`; ohne konfigurierten Token werden Pushes akzeptiert) und `inboundFromWebhook()` (Parsen des Outgoing-Payloads in `ChannelMessage.inbound`; content, senderId=`user_id`, senderName=`user_name`, threadId=`channel_id`, externalId=`post_id`; ein gesetztes `trigger_word` wird aus dem Text entfernt; bei fehlendem `user_id` Fallback `unbekannt`) bereit. Keine neue Dependency (eingebautes `java.net.http.HttpClient` + Jackson 3). Adapter ist als `@Component` mit `@ConditionalOnProperty(jclaw.channels.enabled=true)` registriert. 13 Tests (Verfügbarkeit, Senden inkl. JSON-Body/Pfad/Content-Type und ohne Channel-Override, Fehlerfälle HTTP/fehlende Webhook-URL, Token-Verifikation, Inbound-Parsing inkl. Trigger-Wort-Stripping und Fallbacks).

> **Google-Chat-Adapter (P3-06) getroffen:** `GoogleChatChannelAdapter` implementiert den `ChannelAdapter`-Port für **Google Chat** über Webhooks. Konfiguration im `Channel.config`: `webhookUrl` (Pflicht zum Senden, Incoming-Webhook-URL der Form `https://chat.googleapis.com/v1/spaces/…/messages?key=…&token=…`), `verifyToken` (optional, für die Empfangs-Verifikation). Senden: `POST` des JSON-Payloads `{"text":…}` an den Incoming Webhook, bei HTTP ≥ 400 wird eine `ChannelException` geworfen; die Antwort enthält die erzeugte Nachricht ggf. mit `name` (`spaces/…/messages/…`, wird als `externalId` übernommen; leere/nicht-parsbare Antwort → `externalId` bleibt `null`). Empfang ist **push-basiert** (Google-Chat-Ereignis-Webhooks) — `startReceiving`/`stopReceiving` bleiben Default-No-ops; der Adapter stellt `verifyWebhook()` (Token-Check gegen `verifyToken`; ohne konfigurierten Token werden Pushes akzeptiert) und `inboundFromWebhook()` (Parsen des Ereignis-Payloads in `ChannelMessage.inbound`; content aus `argumentText` – Text nach der Bot-Erwähnung – mit Fallback auf `text` inkl. Erwähnungs-Stripping, senderId/senderName aus `sender.name`/`sender.displayName`, threadId=`space.name`, externalId=`message.name`; Nicht-Nachrichten-Ereignisse wie `ADDED_TO_SPACE` und Payloads ohne Text liefern `null`) bereit. Keine neue Dependency (eingebautes `java.net.http.HttpClient` + Jackson 3). Adapter ist als `@Component` mit `@ConditionalOnProperty(jclaw.channels.enabled=true)` registriert. 13 Tests (Verfügbarkeit, Senden inkl. JSON-Body/Pfad/Content-Type und externalId-Erfassung, leere Antwort, Fehlerfälle HTTP/fehlende Webhook-URL/leerer Inhalt, Token-Verifikation, Inbound-Parsing inkl. Erwähnungs-Stripping und Fallbacks, ignorierbare Ereignisse).

> **Feishu-Adapter (P3-06) getroffen:** `FeishuChannelAdapter` implementiert den `ChannelAdapter`-Port für **Feishu** (Lark) über Webhooks. Konfiguration im `Channel.config`: `webhookUrl` (Pflicht zum Senden, Incoming-Webhook-URL), `verifyToken` (optional, für die Empfangs-Verifikation). Senden: `POST` des JSON-Payloads `{"msg_type":"text","content":{"text":…}}` (Content-Type `application/json; charset=utf-8`) an den Incoming Webhook, bei HTTP ≥ 400 wird eine `ChannelException` geworfen; bei Antwort-`code==0` wird `data.message_id` als `externalId` übernommen (sonst `null`). Empfang ist **push-basiert** (Feishu Event Subscription) — `startReceiving`/`stopReceiving` bleiben Default-No-ops; der Adapter stellt `verifyWebhook()` (Token-Check gegen `verifyToken`; ohne konfigurierten Token werden Pushes akzeptiert) und `inboundFromWebhook()` (Parsen des `event_callback`-Payloads in `ChannelMessage.inbound`; Text-Events nur — `message_type=="text"` mit `content`-JSON → `text`, senderId aus `sender.sender_id.user_id` mit Fallback auf `open_id`/`unbekannt`, threadId=`message.chat_id`, externalId=`message.message_id`; `url_verification`-Challenges, Nicht-Text-Events wie `image` und Payloads ohne Text liefern `null`) bereit. Keine neue Dependency (eingebautes `java.net.http.HttpClient` + Jackson 3). Adapter ist als `@Component` mit `@ConditionalOnProperty(jclaw.channels.enabled=true)` registriert. 15 Tests (Verfügbarkeit, Senden inkl. JSON-Body/Content-Type/externalId-Erfassung und mit ThreadId, Nicht-Null-`code`, Fehlerfälle HTTP/fehlende Webhook-URL/leerer Inhalt, Token-Verifikation, Inbound-Parsing inkl. user_id/open_id-Fallbacks, ignorierbare Events).

> **Synology-Chat-Adapter (P3-06) getroffen:** `SynologyChatChannelAdapter` implementiert den `ChannelAdapter`-Port für **Synology Chat** über Webhooks. Konfiguration im `Channel.config`: `incomingWebhookUrl` (Pflicht zum Senden, URL der Form `https://<synology>:<port>/webhook/<hookId>`), `outgoingWebhookToken` (optional, für die Empfangs-Verifikation), `channel` (optionaler Standard-Channel-Override, z. B. `#general`). Senden: `POST` des JSON-Payloads `{"text":…,"channel_name":…}` an den Incoming Webhook — `channel_name` aus `threadId`/`senderId`, Fallback auf den konfigurierten `channel`; ohne Ziel/Override wird nur `text` gesendet (Webhook-Default); bei HTTP ≥ 400 wird eine `ChannelException` geworfen. Empfang ist **push-basiert** (Synology-Outgoing-Webhook) — `startReceiving`/`stopReceiving` bleiben Default-No-ops; der Adapter stellt `verifyWebhook()` (Token-Check gegen `outgoingWebhookToken`; ohne konfigurierten Token werden Pushes akzeptiert) und `inboundFromWebhook()` (Parsen des Outgoing-Payloads in `ChannelMessage.inbound`; content, senderId=`user_id`, senderName=`user_name`, threadId=`channel_id` mit Fallback auf `channel_name`, externalId=`post_id`; bei fehlendem `user_id` Fallback `unbekannt`) bereit. Neuer `ChannelType.SYNOSOLOGY_CHAT`. Keine neue Dependency (eingebautes `java.net.http.HttpClient` + Jackson 3). Adapter ist als `@Component` mit `@ConditionalOnProperty(jclaw.channels.enabled=true)` registriert. 14 Tests (Verfügbarkeit, Senden inkl. JSON-Body/Pfad/Content-Type, Channel-Fallback-Kette threadId→senderId→config, ohne Override, Fehlerfälle HTTP/fehlende Webhook-URL, Token-Verifikation, Inbound-Parsing inkl. channel_id/channel_name-Fallbacks und Ignorieren ungueltiger Payloads).

> **X-Adapters (P3-06) getroffen:** `XChannelAdapter` implementiert den `ChannelAdapter`-Port für **X (Twitter) Direct Messages** über die X-API-v2-DM-Endpunkte. Konfiguration im `Channel.config`: `token` (OAuth-2.0-User-Context-Token, Pflicht), `baseUrl` (Standard `https://api.x.com/2`), `pollIntervalSeconds` (Standard 30). Senden: `POST {baseUrl}/dm_conversations/with/{participantId}/messages` mit `Authorization: Bearer {token}` und `{"text":…}`; `participantId` aus `threadId`/`senderId`, bei HTTP ≥ 400 wird eine `ChannelException` geworfen; `data.id` wird als `externalId` übernommen. Empfang: `startReceiving` pollt `GET {baseUrl}/dm_events?max_results=100` in einem Daemon-Thread (DM-Events des authentifizierten Kontos, neueste zuerst) — achtet nur auf `event_type=="MessageCreate"` mit Text, liefert die Batch in chronologischer Reihenfolge (aelteste zuerst) und dedupliziert über ein In-Memory-Set der Event-Ids; Mapping in `ChannelMessage.inbound`: content, senderId=`sender_id` (Fallback `unbekannt`), threadId=`dm_conversation_id`, externalId=Event-`id`. Fehler führen zu 5-s-Retry im Loop (Muster: Telegram). Keine neue Dependency (eingebautes `java.net.http.HttpClient` + Jackson 3). Adapter ist als `@Component` mit `@ConditionalOnProperty(jclaw.channels.enabled=true)` registriert. 11 Tests (Verfügbarkeit, Senden inkl. Auth-Header/Pfad/Body/externalId, Fehlerfälle HTTP/fehlender Teilnehmer/fehlendes Token, dm_events-Polling inkl. chronologischer Reihenfolge und Deduplizierung über Polls, Ignorieren von Nicht-`MessageCreate`-Events, Daemon-Loop).

> **ClickClack-Adapter (P3-06) getroffen:** `ClickClackChannelAdapter` implementiert den `ChannelAdapter`-Port für **ClickClack** (REST-API, Bot-User). Konfiguration im `Channel.config`: `token` (Pflicht, Bot-Token als Bearer), `workspace` (Pflicht, Workspace-Name/-Slug), `baseUrl` (Pflicht, Host), `pollIntervalSeconds` (Standard 30), `defaultTo` (optionales Standard-Ziel). Senden: REST mit `Authorization: Bearer {token}` — Ziel-Normalisierung `channel:`/`dm:`/`thread:` (ohne Präfix = Channel), Auflösung der Channel-Id über Workspace-Id (`GET /api/workspaces` → `GET /api/workspaces/{id}/channels`, Name→Id mit Cache), DM-Id über `GET /api/dms` bzw. Neuanlage über `POST /api/dms {workspace_id, member_ids}` (`can_send`-Check), Senden über `POST /api/channels/{id}/messages` / `POST /api/dms/{id}/messages` bzw. `/messages/{id}/thread/replies` für Thread-Antworten; `message.id` als `externalId`; bei HTTP ≥ 400 `ChannelException`. **Empfang ist poll-basiert und nutzt die Ingress-Monitor-Abstraktion (P3-08):** der Adapter stellt einen `IngressPoller` über `GET /api/realtime/events?workspace_id=&after_cursor=&limit=100&include_tail=true`; der erste Lauf bootstrappt per `tail_cursor` (kein History-Replay), danach verarbeitet er `message.created`/`thread.reply_created`-Events (`event.id`=`evt_…` als Store-externalId, Nachricht per `GET /api/messages/{id}`), filtert eigene Bot-Nachrichten (`author_id` = Bot-User-Id aus `GET /api/me`) und hält bei fehlgeschlagenem Nachrichten-Abruf den Cursor an. Adapter ist als `@Component` (`@ConditionalOnProperty(jclaw.channels.enabled=true)`) mit `@Autowired`-Konstruktor für den `ChannelIngressMonitor`-Bean registriert. Keine neue Dependency. 18 Tests (Verfügbarkeit, Senden: Channel/Name-Auflösung/bestehender DM/DM-Neuanlage/Thread-Reply/defaultTo/Fehlerfälle; Empfang: Bootstrap ohne Replay, Event-Reihenfolge, Selbst-Filter, Nicht-Nachrichten-Events, Fetch-Retry ohne Cursor-Advance, Neustart-Dedupe über geteilten Store, Daemon-Loop Start/Stop/Fehlerweiterlauf).
...
> **Ingress-Monitor-Abstraktion (P3-08) getroffen:** Gemeinsamer, dauerhafter Empfangs-Lifecycle für poll-basierte Channel-Adapter nach dem OpenClaw-Plugin-SDK-Vorbild. Der neue `ChannelIngressMonitor` (`@Service`, im `domain.service`-Paket) orchestriert statt eigener Adapter-Logik: (1) **Polling** – der Adapter stellt einen `IngressPoller` bereit (`List<ChannelMessage> poll(String sincePosition)`); der Monitor übergibt die zuletzt gespeicherte Cursor-Position als Startpunkt. (2) **Claim-Identity-Validation** – optionale Vorprüfung (`Predicate<ChannelMessage>`) vor der Übernahme; abgelehnte Claims werden gezählt und übersprungen. (3) **Durable admission** – übernommene Items werden über den neuen `IngressCursorStore`-Port dauerhaft bestätigt (At-most-once über Neustarts hinweg) und der Cursor wird fortgeschrieben; `JdbcIngressCursorStore` (`@Repository`) persistiert in den neuen H2-Tabellen `channel_ingress` (Cursor je Channel) und `channel_ingress_item` (bestätigte Items, PK `(channel_id, item_id)`, in `schema.sql`). (4) **Adoption handoff** – übernommene Nachrichten gehen an den `ChannelAdapter.InboundMessageHandler` (`onMessage`). (5) **Pruning** – `pruneOlderThan(channel, cutoff)` entfernt alte bestätigte Items (Index auf `(channel_id, seen_at)`). (6) **Shutdown** – `start()` liefert eine `IngressSession` (`AutoCloseable`); `stop()` beendet den Daemon-Loop kooperativ (`Interrupt` + `running`-Flag). Konvention: Poller liefern Items in chronologischer Reihenfolge (älteste zuerst); der Cursor zeigt auf das letzte übernommene Item. `pollOnce()` ist synchron für Tests/Webhook-Alternativen nutzbar und liefert `PollResult(newItems, skipped, total)`. Konfig/Steuerung beim Betreiber; Store ungegat wie übrige H2-Stores. 21 Tests (Store: Cursor-Upsert, Dedupe/Admission je Channel, Pruning inkl. Kanal-Isolation, Counting; Monitor: Reihenfolge, Dedupe, fehlende externalId, Claim-Validator, Cursor-Weitergabe als `sincePosition`, Null-Poller-Liste, Loop-Start/Stop, Error-Backoff-Reconnect, Pruning-Delegation; Restart-Durability: zwei Monitor-Instanzen auf demselben H2-Store liefern nach „Neustart“ keine Duplikate, aber neu Hinzugekommene). Volle Suite 672 Tests grün.

## Nächste Schritte

1. **P3-06/P3-08** Weitere Channel-Adapter (**IRC ✅, Email ✅, Mattermost ✅, Google Chat ✅, Feishu ✅, Synology Chat ✅, X ✅, ClickClack ✅**) — verbleibend: **Buzz (Nostr)** (WebSocket + NIP-29/NIP-42 + Signierung) — und die **Ingress-Monitor-Abstraktion** aus dem OpenClaw-Plugin-SDK ist **umgesetzt (P3-08 ✅)**; Adapter können sie nutzen statt eigener Polling-Logik (künftig z. B. Beispiele: X-DM, Telegram, Email, ClickClack). Dabei **P3-07** Media-Message-Support berücksichtigen.
