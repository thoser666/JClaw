...
> Der Fortschritt wird in der [Paritäts-Roadmap](parity-roadmap.md) verfolgt. Stand: P0 (Fundament), **P1-01–P1-12** (Agent-Kern, komplett), **P2-01–P2-06, P2-09** (Konfiguration & Gateway, komplett), **P3-01** (Channel-API, komplett), **P3-02** (Telegram, komplett), **P3-03** (Slack Socket Mode, komplett), **P3-04** (Discord Gateway, komplett), **P3-05** (WhatsApp Cloud API, komplett), **P3-06** (Weitere Channels — IRC-, E-Mail-, Mattermost-, Google-Chat-, Feishu-, Synology-Chat-, X-DM-, ClickClack- und Buzz-(Nostr)-Adapter, **komplett**), **P3-08** (Ingress-Monitor-Abstraktion — durable admission, polling, pruning, claim-identity validation, adoption handoff, shutdown, komplett), **P4-02** (Open Memory Vault — Write/List **und Read-Back/Watcher-Sync**, komplett), **P4-01** (Plugin-Laufzeit — Runtime-Slice: `plugin-sidecar.js` mit `definePluginEntry`/`defineChannelPluginEntry`, Tool-/Command-/Hook-Registrierung zur Laufzeit, `NodeSidecarPluginRuntime` + `EntryPointResolver`, **in Arbeit**). **Phase 1 ist komplett; Phase 2 ist komplett; Phase 3 begonnen (P3-06 komplex fertig, P4-01-Runtime-Slice umgesetzt).** Die Roadmap ist seit 2026-09-02 **thematisch gruppiert** (Agent-Kern · Memory/Persistenz/Sessions · Konfig/Gateway/Control-UI · Automations & Workflows · Channels · Browser/Computer/Talk · Multi-Agent & Orchestrierung · Plugin-Ökosystem · Memory-Wissen/Skill-Workshop) und enthält die OpenClaw-2026.8.x-Features (P3-07 Media-Message-Support, P4-07 Goals & Queues, P4-08 Background-Sessions, P4-09 Plugin-Security, P4-10 Backup & Restore, P4-11 Browser, P4-12 Computer Use, P4-13 Talk, P4-14 Voice/TTS-Personas, P4-15 Shared Memory Pools, P4-16 Stable API).
...
4. ✅ P3-04 Discord — `DiscordChannelAdapter` (Gateway-WebSocket via Jakarta-WebSocket-Client: `GET /gateway` für die WS-URL, `Identify` op 2 beim `Hello` op 10, Heartbeat-Antwort auf op 1, `MESSAGE_CREATE`-Dispatch für Empfang; `POST /channels/{id}/messages` fürs Senden), Konfig `token`/`baseUrl`/`intents`. Zerlegbar/getestet über injizierbaren `WebSocketConnector`+`SessionHandle`. 11 Tests.
5. ✅ P3-05 WhatsApp — `WhatsAppChannelAdapter` (Meta Cloud API: `POST /{phoneNumberId}/messages` fürs Senden; push-basiert für Empfang via Webhook mit `verifyWebhook()`-Handshake und `inboundFromWebhook()`-Parse), Konfig `token`/`phoneNumberId`/`graphUrl`/`verifyToken`. 10 Tests.
6. ✅ P3-06/IRC — `IrcChannelAdapter` (direkte TCP-Verbindung: `NICK`/`USER`/`JOIN` beim Start, `PRIVMSG` fürs Senden, Daemon-Lesethread mit PRIVMSG-Parsing für den Empfang), Konfig `server`/`port`/`nick`/`channel`/`nickservPassword` (optional `PRIVMSG NickServ :IDENTIFY`). Zerlegbar/getestet über injizierbaren `IrcConnector`+`IrcSession` (keine neue Dependency — Standard-Java-Sockets). 10 Tests.
7. ✅ P3-06/Email — `EmailChannelAdapter` (SMTP-Dialog RFC 5321 fürs Senden: `EHLO`/`MAIL FROM`/`RCPT TO`/`DATA`/`QUIT` inkl. Dot-Stuffing; IMAP-Polling RFC 3501 für den Empfang: `LOGIN`/`SELECT`/`UID SEARCH UNSEEN`/`UID FETCH (RFC822)`/`UID STORE +FLAGS (\Seen)`), Konfig `server`/`username`/`password`/`from`/`smtpPort`/`imapPort`/`imapFolder`/`pollIntervalSeconds`/`useTls`. MIME-Parsing via Jakarta Mail (`spring-boot-starter-mail`), text/plain bevorzugt. End-to-End gegen Fake-SMTP/Fake-IMAP-Server getestet. 12 Tests.
8. ✅ P3-06/Mattermost — `MattermostChannelAdapter` (Webhook-basiert: `POST` des JSON-Payloads `{"channel","username","text"}` an den Incoming Webhook fürs Senden; push-basiert für Empfang via Outgoing Webhook mit `verifyWebhook()`-Token-Check und `inboundFromWebhook()`-Parse inkl. Trigger-Wort-Stripping), Konfig `incomingWebhookUrl`/`outgoingWebhookToken`/`channel`/`username`. Keine neue Dependency (`java.net.http.HttpClient` + Jackson 3), gegen In-Process-HTTP-Server getestet. 13 Tests.
9. ✅ P3-06/GoogleChat — `GoogleChatChannelAdapter` (Webhook-basiert: `POST` des JSON-Payloads `{"text"}` an den Incoming-Webhook (`chat.googleapis.com/v1/spaces/…/messages?key=…&token=…`) fürs Senden; push-basiert für Empfang via Ereignis-Webhook mit `verifyWebhook()`-Token-Check und `inboundFromWebhook()`-Parse — content aus `argumentText` mit Erwähnungs-Stripping), Konfig `webhookUrl`/`verifyToken`, `externalId` aus Antwort-`name`. Keine neue Dependency (`java.net.http.HttpClient` + Jackson 3), gegen In-Process-HTTP-Server getestet. 13 Tests.
10. ✅ P3-06/Feishu — `FeishuChannelAdapter` (Webhook-basiert: `POST` des JSON-Payloads `{"msg_type":"text","content":{"text"}}` an den Incoming-Webhook fürs Senden, `externalId` aus `data.message_id` bei `code==0`; push-basiert für Empfang via Event-Subscription (`event_callback`) mit `verifyWebhook()`-Token-Check und `inboundFromWebhook()`-Parse — nur Text-Events, `content`-JSON → text, senderId aus `user_id` mit Fallback auf `open_id`), Konfig `webhookUrl`/`verifyToken`. Keine neue Dependency (`java.net.http.HttpClient` + Jackson 3), gegen In-Process-HTTP-Server getestet. 15 Tests.
11. ✅ P3-06/SynologyChat — `SynologyChatChannelAdapter` (Webhook-basiert: `POST` des JSON-Payloads `{"text","channel_name"}` an den Incoming-Webhook (`/webhook/<hookId>`) fürs Senden, `channel_name` aus threadId/senderId mit Fallback auf config-`channel`; push-basiert für Empfang via Outgoing Webhook mit `verifyWebhook()`-Token-Check und `inboundFromWebhook()`-Parse — threadId aus `channel_id` mit Fallback auf `channel_name`), Konfig `incomingWebhookUrl`/`outgoingWebhookToken`/`channel`; neuer `ChannelType.SYNOSOLOGY_CHAT`. Keine neue Dependency (`java.net.http.HttpClient` + Jackson 3), gegen In-Process-HTTP-Server getestet. 14 Tests.
12. ✅ P3-06/X — `XChannelAdapter` (X-API-v2-DM-Endpunkte: `POST /2/dm_conversations/with/{id}/messages` mit `Authorization: Bearer <token>` fürs Senden; `GET /2/dm_events`-Polling im Daemon-Thread für den Empfang — `MessageCreate`-Events, chronologisch sortiert und über Event-Ids dedupliziert), Konfig `token`/`baseUrl`/`pollIntervalSeconds`. Keine neue Dependency (`java.net.http.HttpClient` + Jackson 3), gegen In-Process-HTTP-Server getestet. 11 Tests.
13. ✅ P3-08 Ingress-Monitor-Abstraktion — `ChannelIngressMonitor` (`@Service`, `biz.brumm.domain.service`): dauerhafter Empfangs-Lifecycle nach SDK-Vorbild. Adapter stellt den `IngressPoller` (`poll(sincePosition)`); der Monitor validiert Claims (`Predicate<ChannelMessage>`), übernimmt Items über den `IngressCursorStore`-Port dauerhaft (At-most-once über Neustarts, Tabelle `channel_ingress_item` mit PK `(channel_id, item_id)`), schreibt den Cursor fort (`channel_ingress`), übergibt übernommene Nachrichten an den `ChannelAdapter.InboundMessageHandler` und unterstützt Pruning (`pruneOlderThan`) sowie kooperativen Shutdown (`IngressSession.stop()`). H2-Implementierung `JdbcIngressCursorStore` (`@Repository`); `pollOnce()` synchron für Tests/Webhooks, `start()` für den Daemon-Loop (Intervall, 5-s-Backoff bei Fehlern). Konvention: Poller liefern chronologisch (älteste zuerst). 21 Tests.
14. ✅ P3-06/ClickClack — `ClickClackChannelAdapter` (ClickClack-REST-API mit Bot-Token: `Authorization: Bearer <token>`; Auflösung über `GET /api/workspaces` → `GET /api/workspaces/{id}/channels` (Name→Id, Cache) bzw. `GET /api/dms`/`POST /api/dms`; Senden über `POST /api/channels/{id}/messages` / `/api/dms/{id}/messages` bzw. `/messages/{id}/thread/replies` für Thread-Antworten, `message.id` als `externalId`; Ziel-Normalisierung `channel:`/`dm:`/`thread:` mit `defaultTo`-Fallback), Konfig `token`/`workspace`/`baseUrl`/`pollIntervalSeconds`/`defaultTo`. **Empfang poll-basiert über die Ingress-Monitor-Abstraktion (P3-08):** `IngressPoller` gegen `GET /api/realtime/events` (Bootstrap per `tail_cursor`, danach `message.created`/`thread.reply_created`, Nachrichten-Abruf per `GET /api/messages/{id}`, Selbst-Filter über `GET /api/me`, Cursor-Halt bei Abruffehler). Keine neue Dependency (`java.net.http.HttpClient` + Jackson 3), gegen In-Process-HTTP-Server getestet. 18 Tests.
15. ✅ P3-06/Buzz — `BuzzChannelAdapter` (NIP-01-WebSocket direkt zum Relay, NIP-29-Gruppen: kind-9-Events mit `h`-Tag; Senden mit BIP-340-Schnorr-Signatur über die Event-Id, Ziele `buzz:<raum>`/`room:<raum>`/bare `<raum>`/`buzz:thread:<raum>:<root>` mit NIP-10-`reply`-Marker; NIP-42-AUTH mit signiertem kind-22242-Event auf `AUTH`-Challenge), Konfig `relay`/`privateKey`/`defaultTo`/`pollIntervalSeconds`/`relayTimeoutSeconds`. **Empfang poll-basiert über die Ingress-Monitor-Abstraktion (P3-08):** `REQ {"kinds":[9],"#h":[…],"since":…}`, Bootstrap ohne History-Replay, Selbst-Filter über die eigene Pubkey, durable externalId `"<created_at>:<event-id>"`. **Neue Dependency: BouncyCastle (`bcprov-jdk18on`)** — Krypto (`NostrCrypto`) gegen den offiziellen BIP-340-Testvektor 0 getestet. 24 Tests.
16. 🔵 P4-01 Node-Sidecar-Plugin-Laufzeit — **Runtime-Slice umgesetzt:** `sidecar/plugin-sidecar.js` führt CommonJS-Entries über die OpenClaw-Entry-Semantik aus (`definePluginEntry`/`defineChannelPluginEntry` in einer `vm`-Sandbox); Tools/Commands/Hooks werden zur Laufzeit registriert (`plugin.load`/`plugin.unload`), `sidecar.listTools` liefert Referenz- + Plugin-Tools, `tool.call` führt `before_tool_call`/`after_tool_call`-Hooks aus (Blocking via `ERROR_HOOK_BLOCKED`). Java: `NodeSidecarPluginRuntime` (opt-in `jclaw.agent.plugins.runtime.enabled=true`) + `EntryPointResolver` (`package.json`→`main` mit Traversal-Schutz, sonst Fallbacks). Dazu Temp-Datei-Start der Bridge (Windows-`CreateProcess`-Längenlimit). Offen: npm/TypeScript-Bundles, Voll-Hook-Katalog, Channel-Runtime, Spring-AI-Tool-Schema-Binding. Danach: P4-07/P4-08 Automations (Goals & Queues, Background-Sessions); P4-11–P4-14 Browser/Talk/Voice aus 2026.8.2; **P4-02 Open Memory Vault** (Vault-Write/List ✅ **und Read-Back/Watcher-Sync ✅**; offen: semantisches Retrieval + Compaction-Trigger) + **P4-15 Shared Memory Pools**; **P4-16 Stable API** als Differenzierung zu OpenClaws breaking-change-Kultur.**
